...
командная оболочка, и второй сценарий, когда загружается система с GUIю
...
- Вывод в консоль содержимого файла `/etc/issue`;
- Запрос у человека данных для входа в систему (имя пользователя и его пароль);
- Запуск командной оболочки, установленной по умолчанию для пользователя, от
