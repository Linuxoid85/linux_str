...
### Сборка пакета из исходного кода

При сборке программного обеспечения из исходного кода пользователь выполняет
следующие действия:

1. **Конфигурирование пакета.** На данный момент ещё большинство пакетов использует
   классические autotools и систему сборки GNU Make. Соответственно, в таком
   случае в пакете присутствует скрипт `configure`, который и выполняет
   конфигурацию: проверяет наличие необходимых файлов (библиотек и прочего) и
   создаёт `Makefile`.
2. **Сборка пакета.** На данном этапе выполняются инструкции из `Makefile` или
   других подобных файлов.
3. **Тестирование пакета.** Это необязательный шаг. Выполняются тесты,
   показывающие, как хорошо пакет будет работать в системе. Как правило
   предназначен для разработчиков, но будет полезен тем, кто, например, собирает
   свой дистрибутив GNU/Linux с нуля из исходного кода.
4. **Установка пакета в систему**. Копирование собранных файлов в систему. Для
   многих пакетов можно изменить путь для установки, указав иное значение
   параметра `DESTDIR`, `PREFIX` или какого-нибудь подобного.
5. **Настройка и выполнение прочих действий после установки.** Как правило -
   перемещение некоторых файлов, которые были установлены не в те директории, в
   нужные места, создание конфигов и прочие действия. *Данный пункт встречается
   не так часто и в некоторых случаях не является обязательным*.
